...
class servidor(object):
    ...
    def create_server(self):
        ...
        #uri2 = daemon.register(jglobal) #TODO mudar a função
        #ns.register("example.window", uri2)
        daemon.requestLoop()
...
class cliente(object):
    ...
    def connect(self):
        ...
        #wind = Pyro4.Proxy("PYRONAME:example.window") 
...
class janela(object):
    ...
    def imprime_jogo(self, dik):
        self.j.screen.addstr(0, 0, self.j.labels['l_score'][0]+': '+str(dik['score']))
        self.j.screen.addstr(0, self.j.labels['l_won'][1], self.j.labels['l_won'][0]+': '+str(dik['won']))
        self.j.screen.addstr(self.j.y-1, 0, self.j.labels['l_round'][0]+": "+str((dik['round'])))
        self.j.screen.addstr(self.j.y-1, self.j.labels['l_lifes'][1], self.j.labels['l_lifes'][0]+": "+str(dik['lifes']))
        for i in dik['bullets']:
            self.j.screen.addstr(i[2], i[1], 'O')
        self.j.gamepos[dik['nickname']] = (dik['kx'],dik['ky'])
        ...
        if dik['ky']  > self.j.cy:
            ...
            symb = '▼'
            esymb = "▲"
        self.j.screen.addstr(dik['ky'], dik['kx'], symb)
        try:
            self.j.screen.addstr(dik['eky'], dik['ekx'], esymb)
        except:
            pass

        self.j.screen.move(dik['ky'], dik['kx'])
                ...
                if c[0]:
                    ...
                    if self.playes[c[1]].lifes == 0:
                        if (self.players[c[2]].up_won() == 3):
                            break
        '''
    def give(self, nickname):
        d = {}
        d['nickname'] = nickname
        d['bullets'] = self.bullets
        d["score"] = self.players[nickname].score
        d["won"] = self.players[nickname].won
        d['round'] = self.round
        d["lifes"] = self.players[nickname].lifes
        d["kx"] = self.players[nickname].cursorx
        d["ky"] = self.players[nickname].cursory
        p = list(self.players.keys())
        for i in self.players:
            if i != nickname:
                 d["ekx"] = self.players[i].cursorx
                 d["eky"] = self.players[i].cursory
                 break
        return d
...
def exec(jogo, nickname):
    global jglobal
    k = 0
   # jogo.set_win(jglobal)
    jogo.player_enter(nickname)
    j = jglobal
    while jogo.ret('win'):
        #jogo.ret('j.screen.clear()')
        jglobal.screen.clear()
        jglobal.imprime_jogo(jogo.give(nickname))
